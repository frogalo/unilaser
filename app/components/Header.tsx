        ...
        <div className="hidden md:flex items-center gap-8">
          {content.nav.links.map((link, index) => (
            <div key={index} className="relative group py-2">
              ...
              {link.sublinks && (
                <div className="absolute top-full mt-0 left-0 w-48 bg-white shadow-xl rounded-lg border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 py-2 flex flex-col z-50">
                  {link.sublinks.map((sublink, subIndex) => (
                    <Link 
                      key={subIndex} 
                      href={sublink.href}
                      className="px-4 py-2 text-sm font-headline font-semibold text-slate-600 hover:text-blue-900 hover:bg-slate-50 transition-colors"
                    >
                      {sublink.label}
                    </Link>
                  ))}
